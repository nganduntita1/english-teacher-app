...
CREATE TABLE IF NOT EXISTS vocabulary_daily_tracking (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  vocabulary_id UUID REFERENCES vocabulary(id) ON DELETE CASCADE,
...
ALTER TABLE IF EXISTS vocabulary ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS learned_vocabulary ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS vocabulary_daily_tracking ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Lessons are readable by all" ON lessons;
...
DROP POLICY IF EXISTS "Vocabulary is readable by all" ON vocabulary;
...
DROP POLICY IF EXISTS "Quiz questions are readable by all" ON quiz_questions;
...
DROP POLICY IF EXISTS "Users can read their own learned vocabulary" ON learned_vocabulary;
CREATE POLICY "Users can read their own learned vocabulary"
  ON learned_vocabulary
  FOR SELECT
  USING (auth.uid()::text = user_id::text);

DROP POLICY IF EXISTS "Users can insert their own learned vocabulary" ON learned_vocabulary;
CREATE POLICY "Users can insert their own learned vocabulary"
  ON learned_vocabulary
  FOR INSERT
  WITH CHECK (auth.uid()::text = user_id::text);

DROP POLICY IF EXISTS "Users can delete their own learned vocabulary" ON learned_vocabulary;
...
DROP POLICY IF EXISTS "Users can read their own daily tracking" ON vocabulary_daily_tracking;
CREATE POLICY "Users can read their own daily tracking"
  ON vocabulary_daily_tracking
  FOR SELECT
  USING (auth.uid()::text = user_id::text);

DROP POLICY IF EXISTS "Users can insert their own daily tracking" ON vocabulary_daily_tracking;
CREATE POLICY "Users can insert their own daily tracking"
  ON vocabulary_daily_tracking
  FOR INSERT
  WITH CHECK (auth.uid()::text = user_id::text);

DROP POLICY IF EXISTS "Users can delete their own daily tracking" ON vocabulary_daily_tracking;
